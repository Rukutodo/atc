...
import { Plus, UserPlus, Phone, Mail, GraduationCap, Laptop, Home, CheckCircle } from 'lucide-react';
      ...
      {showAddForm && (
        <form onSubmit={handleAddStudent} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <input 
            ...
            required
            className="px-4 py-2 rounded-lg border dark:bg-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-teal-500"
            value={formData.name}
          ...
          <button 
            ...
            className="bg-slate-900 dark:bg-teal-600 text-white px-4 py-2 rounded-lg font-bold hover:opacity-90 disabled:opacity-50"
          >
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {students.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-6 py-12 text-center text-slate-500 italic">No students enrolled yet.</td>
              </tr>
            ) : (
